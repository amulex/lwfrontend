...
export type OnSignal_SessionFrom = (
  type: string | undefined,
  handle: HandleSignalJsonEvent,
...
export const onSignal = (session: Session) => (onlyFrom?: Connection | 'notOwn') => (
  ...
  handle: HandleSignalJsonEvent,
  reviver: JsonParseReviver = dateReviver,
) => {
  ...
  session.on(fullType, event => {
    ...
    if (
      (onlyFrom === 'notOwn' && session.connection.connectionId === from.connectionId) ||
