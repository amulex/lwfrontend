...
export class LiveWidget {
  ...
  public async createParticipant<K extends ParticipantType>(
    ...
    const handleVideoCreated = combineProcedures(
      handleMetadata,
      addButtons,
      getProp(elements.handleTargets, 'created') || noop,
    );
    ...
    const allToAllConnect = this.allToAllConnectSessionMetafactory(
      connectToSession,
      profile.settings,
      elements.streamsTargets,
      signals,
      handleVideoCreated,
      handleVideoDestroyed,
    )(bindTransportAgents);
   ...
   */
  private allToAllConnectSessionMetafactory(
    decorated: ConnectSessionFactory,
    settings: Settings,
    targets: StreamsTargets,
    signals: CallSignals,
    videoCreated: HandleVideoElementEvent = noop,
    videoDestroyed: HandleVideoElementEvent = noop,
  ): PublishersConnectSessionFactory {
